...
impl Instruction {
	...
	pub fn fetch_instruction(cpu: &mut WhiskerCpu) -> Result<(Instruction, u64), TrapRequestGuaranteed> {
		let pc = cpu.pc;
		...
		if parcel1 == 0 {
			...
			return Err(cpu.request_trap(TrapIdx::ILLEGAL_INSTRUCTION, parcel1.extend()));
		}
		...
		if extract_bits_16(parcel1, 0, 1) != 0b11 {
			if support_compressed {
				match insn16::parse(cpu, parcel1) {
					...
					None => {
						...
						Err(cpu.request_trap(TrapIdx::ILLEGAL_INSTRUCTION, parcel1.extend()))
					}
				}
			} else {
				log!(
			...
			match insn32::parse(cpu, full_parcel) {
				Some(insn) => Ok((insn, 4)),
				None => Err(cpu.request_trap(TrapIdx::ILLEGAL_INSTRUCTION, full_parcel.extend())),
			}
